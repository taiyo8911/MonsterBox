...
| ボックス操作 | タップ選択→メニュー（移動／強さを見る／編集する／にがす） |
...
### 4.1 登録フォーム（個体の追加・編集）
- 全項目を入力: ニックネーム／ポケモン名（種族）／性別／レベル／能力値6／性格／**とくせい**／技（最大4）／メモ
...
- 追加時はボックス番号・スロットを先頭の空きマスに自動割り当てして保存

### 4.2 一覧表示
- ソート可能: 図鑑番号 / レベル / 名前 / タイプ / 性格
- 行タップで個体詳細（4.4）へ
- 行を左スワイプで「にがす」（確認アラート → 削除）
- 所持0体のときは `ContentUnavailableView` で誘導文を表示
...
### 4.4 個体詳細（強さを見る）
- 種族・タイプ・レベル・能力値6・性格・とくせい・ニックネーム・ボックス位置・メモ・覚えている技（日本語名）を表示
- 右上「編集する」から登録フォーム（編集）へ
- 画面下部に **にがす** ボタン（確認アラート → 削除）

### 画面構成
- 初回起動: スプライト先読み（進捗表示）→ 完了後にホームへ。一部失敗時は「再取得」または「このまま進む」を選択可
- 一覧／ボックス 切替ビュー（ホーム）／ 個体詳細 ／ 追加・編集フォーム
- ホーム右上の「+」で新規登録。所持数が 14×30=420 体に達している場合は「ボックスが満杯です」アラートを表示して登録を抑止
...
### ユーザーデータ（編集可）
- **OwnedPokemon**: speciesDex / nickname / gender / level / 能力値6 / nature / **abilityID** / moveIDs[最大4] / boxNumber（1〜14）/ slot（0〜29）/ isShiny・memo（任意）/ createdAt
