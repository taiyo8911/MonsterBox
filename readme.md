...
| 一覧ソートキー | 図鑑番号・レベル・名前・タイプ・性格・持ち物 |
...
### 4.1 登録フォーム（個体の追加・編集）
- 全項目を入力: ニックネーム／ポケモン名（種族）／性別／レベル／能力値6／性格／持ち物／技（最大4）
- **種族を選ぶと、技候補がその種族の学習セットに限定される**（`MasterData.learnableMoves(forDex:)`）
- ボックス番号・スロットを割り当てて保存

### 4.2 一覧表示
- ソート可能: 図鑑番号 / レベル / 名前 / タイプ / 性格 / 持ち物
- 行タップで個体詳細（4.4）へ
...
### 4.4 個体詳細（強さを見る）
- 種族・タイプ・レベル・能力値6・性格・持ち物・覚えている技（日本語名）を表示
- 編集フォームへの導線
...
### ユーザーデータ（編集可）
- **OwnedPokemon**: speciesDex / nickname / gender / level / 能力値6 / nature / heldItem / moveIDs[最大4] / boxNumber（1〜14）/ slot（0〜29）/ isShiny・memo（任意）/ createdAt
