...
| ボックス操作 | タップ→上段に詳細表示／長押し→メニュー（つかむ／つよさをかえる／にがす）。長押し時にハプティクス |
...
- 右上「+」で新規登録。所持数が 14×30=420 体に達している場合は「ボックスが満杯です」アラートを表示して登録を抑止

### 4.3 上段: 個体詳細パネル
- 下段のポケモンをタップすると、その個体のデータがここに表示される
- **左カラム**: レベル ／ ニックネーム or 種族名 ／ 性別マーク → スプライト → タイプ → せいかく → とくせい
- **右カラム**: 能力値6（HP / こうげき / ぼうぎょ / とくこう / とくぼう / すばやさ）→ 覚えている技（最大4）
- 未選択時は中身を空に（枠と背景のみ）
...
- **個体の操作**:
  - **タップ**: 上段の詳細パネルに表示。空マスのタップは無視
  - **長押し（0.5秒）**: アクションメニュー表示。ハプティクスで触覚通知
    - **つかむ**: そのポケモンを「手に持った」状態になり、元マスは空く。空きマスをタップで置く。占有マスをタップすると、そこへ置きつつ元の子を持ち上げる（**持ち替え**）。空きマスに置くまで継続。移動モード中に左右ボタンで別ボックスへ移れば**箱間移動**も可。手放す前のキャンセルで元に戻せる
    - **つよさをかえる**: 登録フォーム（編集）
...
- ホーム（上段=詳細パネル / 下段=ボックス）／ 追加・編集フォーム
...
- **MonsterBox/**（プロジェクトルート）
  ...
  - **MonsterBox/**（ソース）
    ...
    - **Features/** — 機能＝タブ単位
      - **Pokemon/** ← Phase 1
        - `PokemonHomeView.swift` — ホーム（上段=詳細パネル / 下段=ボックス）
        - `PokemonDetailPanel.swift` — 上段の個体詳細パネル（左右2カラム）
        - `BoxView.swift` — 14箱・6×5・左右切替（タップ→上段表示／長押し→メニュー）
