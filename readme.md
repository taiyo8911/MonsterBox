...
- 未選択時もパネル形状は維持される（各セルが `—` のプレースホルダ表示）
- 右上「+」で新規登録。**空マスを選択しているときのみ有効**（未選択時は無効）。押下するとそのマスに保存される

### 4.3 上段: 個体詳細パネル
- 下段のポケモンをタップすると、その個体のデータがここに表示される
- **上下2段構成**。常に同じ高さを保つ（空マス選択時・未選択時も同一形状）
- **上段-左**: スプライト → Lv ／ ニックネーム or 種族名 ／ 性別マーク → タイプ（中央揃え）
- **上段-右**: 能力値6（HP / こうげき / ぼうぎょ / とくこう / とくぼう / すばやさ）→ せいかく → とくせい（値は右揃え）
- **下段**: 覚えている技を最大4行（全幅）。各行に 技名 / タイプバッジ / 威力・命中・PP を表示し、Gridで列が縦に揃う
- 性別マークはオス「♂」／メス「♀」のみ表示。性別不明はマークなし
- 未選択時は各セルが `—` / 透明スプライト / 非表示バッジ になり、パネルサイズは変わらない
  ...
  - **MonsterBox/**（ソース）
    ...
    - **Features/** — 機能＝タブ単位
      - **Pokemon/** ← Phase 1
        ...
        - `PokemonDetailPanel.swift` — 上段の個体詳細パネル（上下2段: プロフィール+能力 / 技4行）
